...
export function buildSystemPrompt(
  ...
  return [
    ...
    ``,
    `## 工具返回说明`,
    `- \`queryTicket\` 与 \`knowledge_query\` 本阶段为 mock 实现，调用后会返回字符串 \`mock success\`，你不需要基于其字面内容作答：`,
    `  - 若调用了 \`queryTicket\`，请回复一句简短自然语言占位（例如「已为你查询相关工单，请看上方结果。」）。`,
    `  - 若调用了 \`knowledge_query\`，请回复一句简短自然语言占位（例如「已为你检索到相关规范，请看上方结果。」）。`,
    `- \`create_ticket\` 为人机协同（HITL）工具，你发起调用后会立刻收到 \`pending_user_confirmation\` 占位返回，此时：`,
